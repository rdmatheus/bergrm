...
#'
#' @param formula description of the model, of the type \code{y ~ x} for covariates in mean only, or \code{y ~ x | z} to enter covariates in the dispersion index.
...
#'
#' @return  The \code{bergrm} function returns an object of class "bergrm", which consists of a list with the following components.
#' \describe{
#'   \item{coefficients}{ a list containing the elements "mean" and
#'         "dispersion" that consist of the estimates of the
#'          coefficients associated with the mean and the dispersion index, respectively.}
#'   \item{link, link.phi}{the link function used for the mean model, and for the dispersion index model, respectively.}
...
#'   \item{n.obs, p, k}{Sample size, number of coefficients in the mean model, and number of coefficients in the dispersion index model, respectively.}
...
# #'
# #' # OBS: It is recommended to use the robust boxplot, available by
# #' # the adjbox function of the robustbase package.
# #'
# #' # Fit of the BerG regression model with varying dispersion
# #' fit_disp <- berg_rm(birds ~ when + grazed | when + grazed, data = grazing)
# #' summary(fit_disp)
# #'
# #' # Fit of the BerG regression model with constant dispersion
# #' fit <- berg_rm(birds ~ when + grazed, data = grazing, link.phi = "identity")
# #' summary(fit)
# #'
# #' # Diagnostic
# #' plot(fit)
